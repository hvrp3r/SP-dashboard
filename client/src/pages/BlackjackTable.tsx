          ...
          {history.length === 0 ? (
            ...
            <ul className="space-y-2">
              {history.map((h) => {
                ...
                const handFinal = h.status === 'busted' ? 'Dépassé' : handValue(h.cards);
                return (
                  <li
                    key={h.id}
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-zinc-300 truncate flex items-center gap-1">
                          ...
                          />
                          {h.outcome === 'blackjack' && (
                            <span className="ml-1 text-[10px] px-1.5 py-0.5 rounded bg-amber-500/15 text-amber-400 font-medium uppercase tracking-wide">
                              Blackjack
                            </span>
                          )}
                        </p>
                        <p className="text-xs text-zinc-500">
                          Mise {h.bet_amount} SP · {handFinal} · Croupier : {dealerFinal}
                        </p>
                      </div>
                    </div>
                    <span

...
/** Une soumission = une seule case (index + chiffre), plus jamais une grille entière — voir migration 065. */
 ...
 * déjà validée correcte par le joueur (verrouillée, comme un indice) et '0'
 * ailleurs — c'est la source de vérité pour reconstruire la grille après un
 * rechargement ; le reste (saisies pas encore validées) n'est qu'un
 * brouillon local (localStorage côté client, jamais fait confiance côté
 * serveur).
 */
